...
all_qual <- all %>%
  select(stateabbr, Vote_Clinton, PEItype, lawsunfair2, favoredincumbent2, citizens2, managed, votinginfo, fairofficials, legalelections, bfavored2, oppprevent2, womenopp, minorityopp, leaderselect2, rallies2, newspapers, tv2, fairaccess, faircoverage, donations, rich2, violence2, fraudulent2, easy, choice, postal, expats, internet, rigged2, machinesaccurate, recordssecure, waited2, multiple2, regrestrictive2, popularwill, timely)
...
ui <- navbarPage(h2("Let's Look at Some Relationships!"),
  ...
  fluidRow(
    column(5,
      selectInput("x", "X-Axis", names(all_qual)[3:35]),
      selectInput("y", "Y-Axis", names(all_qual)[3:35], names(all_qual)[4])
    ),
    column(2,
  ...
  fluidRow(
    h3("Variable Key"),
  p(tags$b("Note:"), "For all these variables, experts were asked to rate the various states on a scale of 1-5, which is why we called them categorical. However, all the experts' ratings were averaged, allowing them to be plotted as quantitative.")
  )
  )
)
...
server <- function(input, output) {
  ...
  output$quant <- renderPlotly({
    ...
    ggplot(reactive_all, aes(color = Vote_Clinton, "State" = stateabbr)) +
      geom_point(aes_string(x = input$x, y = input$y)) +
      geom_text(data = reactive_state, mapping = aes_string(x = input$x, y = input$y), label = input$state) +
      scale_color_manual(values = c("red", "blue"), labels = c("Trump", "Clinton"), name = "") +
