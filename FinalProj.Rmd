...
cw_data <- data.frame(state = all$stateabbr, index = all$PEIIndexi, clinton = all$Vote_Clinton, pct_dis = all$Pct_Felon.Disenfranchised, prin_comp$x) 
...
ui <- fluidPage(
  ...
  fluidRow(
    column(3,
      selectInput("x", "X-Axis", names(shiny_all)[3:11]),
      selectInput("y", "Y-Axis", names(shiny_all)[3:11], names(shiny_all)[4])
    ),
    column(4,
      checkboxGroupInput("candidate", "Candidate Chosen", 
                         choices = list("Clinton" = "Yes", "Trump" = "No"),
                         selected = unique(shiny_all$Vote_Clinton))
            ),
    column(5,
      selectInput("state", "What state do you want to see?",
                  choices = unique(shiny_all$stateabbr))
           )
  )
)
...
server <- function(input, output) {
  ...
  output$plot <- renderPlotly({
    req(input$candidate)
    all_reactive <- filter(shiny_all, Vote_Clinton %in% input$candidate)
    state_reactive <- filter(shiny_all, stateabbr %in% input$state)
    ggplot(all_reactive, aes(color = Vote_Clinton)) +
